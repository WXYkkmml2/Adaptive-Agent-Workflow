...
On macOS, if the Python installation lacks a CA bundle and the preflight reports
`CERTIFICATE_VERIFY_FAILED`, set `SSL_CERT_FILE=/etc/ssl/cert.pem` in the same
shell and rerun the preflight. For a company proxy, use its trusted CA bundle
instead. The client keeps hostname and certificate verification enabled.

The script checks API connectivity before the batch, then validates the fixture offline before any agent call. The witness is never included in a prompt or tool catalog. The thresholds and temperature are fixed in `config/settings.py`. Each repeat runs `hierarchical`, `two_layer`, and `two_layer_full_restart` on fresh copies of the same case39 fault. All three use the same candidate validator, sequential joint simulation gate, real action interface, and global goal check. Only the hierarchical method narrows the catalog to the local zone and retains completed region actions after a deviation. The full restart baseline resets the physical state but keeps the cumulative action log and six-action budget.
